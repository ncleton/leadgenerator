...
from lead_studio.integrations.status import (
    check_integrations as read_integration_statuses,
)
from lead_studio.profiles.offers import ResearchProfile, load_profiles, save_profile
from lead_studio.profiles.migration import migrate_offer_profiles_to_objectives
...
from lead_studio.research.browser import collect_html, html_to_markdown
...
from lead_studio.research.visuals import (
...
class LeadStudioServer(MCPServer):
...
def _enforce_allowed_host(url: str) -> None:
    ...
    allowed_host = os.environ.get("LEAD_STUDIO_ALLOWED_HOST", "")
...
def scrape_public_page(url: str) -> dict[str, str]:
    ...
    browser_mode = os.environ.get("LEAD_STUDIO_BROWSER_MODE", "fast")
...
def _interface_status(preferences: LeadStudioPreferences) -> dict[str, object]:
...
@server.tool(
    ...
    description=(
        ...
        "presentation mode. For one explicit NAF/APE code, prefer "
        "search_companies_by_naf. Only call a render tool when chat_ui is enabled."
    ),
    annotations=ToolAnnotations(
        readOnlyHint=True,
        destructiveHint=False,
...
@server.tool(
    ...
    description=(
        ...
        "websites remain missing until separately qualified."
    ),
    annotations=ToolAnnotations(
        readOnlyHint=True,
        destructiveHint=False,
...
@server.tool(
    ...
    description=(
        ...
        "and sourced outreach angle. It only presents supplied data and never "
        "spends credits, sends outreach, or synchronizes it."
    ),
    annotations=ToolAnnotations(
        readOnlyHint=True,
        destructiveHint=False,
...
@server.tool(
    ...
    description=(
        ...
        "integration states, and the HubSpot review. It only presents supplied "
        "data; UI actions continue in chat and never authorize a paid lookup or "
        "CRM write."
    ),
    annotations=ToolAnnotations(
        readOnlyHint=True,
