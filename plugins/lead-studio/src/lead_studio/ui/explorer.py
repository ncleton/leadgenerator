...
LEAD_EXPLORER_UI_URI = "ui://lead-studio/lead-explorer/v9.html"
LEAD_EXPLORER_LEGACY_UI_URIS = tuple(
    f"ui://lead-studio/lead-explorer/v{version}.html" for version in range(1, 9)
)
...
<body>
  ...
  <script>
    (() => {
      ...
      const mappedLeads = () => (data?.leads || []).filter(lead =>
      ...
      function persistState() {
        const state = { activeView, selectedLeadIds:[...selected], activeLeadId };
        ...
        app.innerHTML = `
          <header class="topbar">
            ...
            <div class="summary"><span class="pill"><strong>${esc(total)}</strong> résultat${total === "1" ? "" : "s"}</span>
              <span class="pill"><strong>${mapped}</strong> cartographié${mapped === 1 ? "" : "s"}</span></div>
          </header>
          <nav class="tabs" aria-label="Vues des leads">
            ${tabButton("map","Carte",mapped)}
            ${tabButton("street_map","Carte détaillée",mapped)}
            ${tabButton("naf_list", query.code ? `NAF ${esc(query.code)}` : "Liste", data.leads.length)}
      ...
      function switchView(view, save=true) {
        ...
        if (view === "naf_list") renderList(data.leads, "naf-list-view");
        if (view === "shortlist") renderList(data.leads.filter(lead => selected.has(lead.id)), "shortlist-view", true);
      ...
      async function toggleFullscreen(view=activeView) {
        ...
        try {
          ...
          if(window.leadStudioMcpApp?.connected) {
            const result=await window.leadStudioMcpApp.request("ui/request-display-mode",{mode:target});
            setDisplayMode(result?.mode||target); return;
          }
      ...
      async function followUp(prompt) {
        ...
        if(window.leadStudioMcpApp?.connected) {
          await window.leadStudioMcpApp.request("ui/message",{role:"user",content:{type:"text",text:scoped}}); return;
        }
      ...
      window.addEventListener("lead-studio:host-context", event => applyHostGlobals(event.detail||{}));
      ...
      applyHostGlobals(window.leadStudioMcpApp?.hostContext||{});
      hydrate(window.openai?.toolOutput||window.leadStudioMcpApp?.toolResult?.structuredContent||window.openai);
