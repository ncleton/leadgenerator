...
LEAD_WORKSPACE_UI_URI = "ui://leadgenerator/lead-workspace/v7.html"
LEAD_WORKSPACE_LEGACY_UI_URIS = tuple(
    f"ui://leadgenerator/lead-workspace/v{version}.html" for version in range(1, 7)
)
...
<body>
  ...
  <script>
    (() => {
      ...
      async function followUp(prompt,applyScope=true) { const scoped=applyScope?scopedPrompt(prompt):prompt;if(typeof window.openai?.sendFollowUpMessage==="function") return window.openai.sendFollowUpMessage({prompt:scoped,scrollToBottom:true});if(window.leadGeneratorMcpApp?.connected)return window.leadGeneratorMcpApp.request("ui/message",{role:"user",content:{type:"text",text:scoped}});alert("Poursuivez dans le chat : "+scoped); }
      ...
      applyGlobals(window.openai||{});applyGlobals(window.leadGeneratorMcpApp?.hostContext||{});hydrate(window.openai?.toolOutput||window.leadGeneratorMcpApp?.toolResult?.structuredContent||window.openai);
