...
class LeadGeneratorPreferences(BaseModel):
...
def load_preferences(
    ...
    if not path.exists():
        return LeadGeneratorPreferences()
    return LeadGeneratorPreferences.model_validate_json(
        path.read_text(encoding="utf-8")
    )


def save_preferences(
    preferences: LeadGeneratorPreferences,
    profile_home: Path | None = None,
) -> Path:
